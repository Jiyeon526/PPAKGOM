  ...
  <el-row :gutter="24">
    <el-col :span="12">
      ...
      >
        ...
        <el-table-column prop="study_id" label="방번호"> </el-table-column>
        <el-table-column prop="study_name" label="스터디명"> </el-table-column>
        <el-table-column prop="owner_user_name" label="방장명">
        </el-table-column>
        <el-table-column prop="state" align="right">
          <template #default="scope">
            ...
            <div v-else>
              <el-button
                size="mini"
                type="danger"
                @click="handleCheckJoinReject(scope.$index, scope.row)"
                >가입거절</el-button
              >
            </div>
          </template>
    ...
    <el-col :span="12">
      ...
      >
        ...
        <el-table-column prop="studyId" label="방번호"> </el-table-column>
        <el-table-column prop="studyName" label="스터디명"> </el-table-column>
        <el-table-column prop="userName" label="방장명"> </el-table-column>
        <el-table-column prop="state" align="right">
          <template #default="scope">
            <div v-if="scope.row['state'] == 2">
              <el-button
                size="mini"
                type="primary"
                @click="inviteAccept(scope.$index, scope.row)"
                >승인</el-button
              >
              <el-button
                size="mini"
                type="danger"
                @click="inviteReject(scope.$index, scope.row)"
                >거절</el-button
              >
            </div>
          </template>
  ...
  setup(props, { emit }) {
    ...
    const handleClick = async function(row, column, cell, event) {
      if (column.property != "state") {
        const wantOwner = await store.dispatch(
          "root/requestNameUserJoinStudyList",
          row["owner_user_name"]
        );
        state.inStudyList = [];
        state.inStudyList = wantOwner.data;
        store
          .dispatch("root/requestOtherProfile", row["owner_user_name"])
          .then(function(res) {
    ...
    const handleUserClick = async function(row, column, cell, event) {
      if (column.property != "state") {
        ...
        state.inStudyList = [];
        state.inStudyList = wantOwner;
        store
          .dispatch("root/requestOtherProfile", row["userName"])
          .then(function(res) {
            const profileData = res.data;
            const origin_url = profileData["profile_thumbnail"];
            const need_from = origin_url.indexOf("image");
    ...
    const handleCheckJoinReject = async function(index, row) {
      ...
      state.ownerId = wantOwner.data.studyResult[0]["owner_id"];
      store
        .dispatch("root/requestCheckJoinReject", {
          study_id: state.studyId,
          user_id: state.ownerId
        })
        .then(function(res) {
    ...
    const askJoinList = function() {
      store.dispatch("root/requestAskJoinList").then(function(res) {
        state.joinList = res.data;
      });
    };

    const inviteReceiveList = function() {
      store.dispatch("root/requestinviteReceiveList").then(function(res) {
    ...
    const inviteAccept = async function(index, row) {
      ...
      store
        .dispatch("root/requestSendAccept", body)
        .then(function(res) {
          console.log("!!!!", res);
        })
        .catch(function(err) {
    ...
    const inviteReject = async function(index, row) {
      ...
      body.append("studyId", state.studyId);
      body.append("senderId", state.ownerId);
      store.dispatch("root/requestSendReject", body).then(function(res) {});
      state.receiveList.splice(index, 1);
    };
