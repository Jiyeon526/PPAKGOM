<template>
  ...
  >
    ...
    <div style="display:flex; align-items:center">
      <el-image style="width: 50%; height: 50%;" :src="state.uri" :fit="fit">
      </el-image>
      <div style="width:50%; height:50%; text-align:center">
        <h2 style="margin:30px">
          {{ selectStudy.name }}
          <i
            v-if="state.likeStudy"
            @click="clickLikeCancleBtn"
            class="fas fa-heart"
            style="color:red"
          ></i>
          <i
        ...
        <h4>
          모집 인원 : {{ selectStudy.joined_population }}/{{
            selectStudy.population
          }}
        </h4>
    ...
    <div>
      ...
      <p
        style="text-align:right; font-weight:bold; margin-top:2px; margin-bottom:2px;"
      >
        {{ selectStudy.temperature }}℃
      ...
      <el-progress
        ...
        show-text="false"
        :stroke-width="24"
        :percentage="selectStudy.temperature"
        status="success"
    ...
    <el-divider style="margin: 5px"></el-divider>
    <div class="detail-dialog-footer" v-if="state.isLoggedIn">
      <el-button
        v-if="selectStudy.enter"
        type="success"
        plain
        style="height: 30px"
        @click="enterStudy"
    ...
    watch(
      ...
      () => {
        ...
        if (props.selectStudy.temperature <= 20) {
          ...
          35 <= props.selectStudy.temperature &&
          props.selectStudy.temperature <= 45
        ) {
          state.customColor = "#e6a23c";
          state.iconTemp = 2;
        } else if (
          45 <= props.selectStudy.temperature &&
          props.selectStudy.temperature <= 60
        ) {
          state.customColor = "#5cb87a";
          state.iconTemp = 2;
        } else if (
          60 <= props.selectStudy.temperature &&
          props.selectStudy.temperature <= 80
        ) {
